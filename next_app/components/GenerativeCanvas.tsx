...
function geometryFor(variant: CanvasVariant, seed = 0): RawSegment[] {
  ...
  // por abajo). Merlones grandes (≈2× el hueco) con trazos largos para que no se
  // rompan en guiones con el desvanecido de la tinta.
  const battlement = (x0: number, x1: number, y: number, h: number, n: number, accent = false) => {
    line(x0, y, x1, y, accent);
    const total = x1 - x0;
    const mw = total / (1.5 * n - 0.5);
    const gap = mw * 0.5;
    for (let i = 0; i < n; i++) {
...
export default function GenerativeCanvas({
  ...
  useEffect(() => {
    ...
    const phaseAt = (t: number): { formStrength: number; fade: number } => {
      ...
      // Velo de fondo: muy tenue cuando la figura está formada para que se asiente
      // sólida (sin parpadeo de los trazos cortos), más fuerte al dispersar
      const fade = 0.02 - 0.016 * formStrength;
